...
trap 'log "macOS onboarding failed on line $LINENO"; exit 1' ERR

require_macos

log "Starting AutoShop macOS onboarding"
...
environment="$(prompt_value "Environment (prod/staging)" "staging")"
case "$environment" in
  prod|staging)
    ;;
  *)
...
runtime_root="$ROOT_DIR/.runtime/macos"
export AUTOSHOP_HOME="$runtime_root"
export ENV_DIR="$runtime_root/env/$environment"
export DATA_ROOT="$runtime_root/data/$environment"
export BACKUP_ROOT="$runtime_root/backups"
...
if [[ -n "$existing_bootstrap_password" ]]; then
  ...
  case "$(lowercase "$keep_existing_password")" in
    yes|y) admin_password="$existing_bootstrap_password" ;;
    no|n) admin_password="$(prompt_secret "Admin password")" ;;
...
crm_port="$(prompt_value "CRM port" "${existing_crm_port:-5173}")"
client_port="$(prompt_value "Client WEB port" "${existing_client_port:-5174}")"
server_access_point="$(prompt_value "Server access point / demo hostname" "${existing_server_access_point:-localhost}")"
...
case "$(lowercase "$install_docker_choice")" in
  yes|y) "$SCRIPT_DIR/install-docker-macos.sh" ;;
  no|n) log "Skipping Docker install by user choice" ;;
  *) die "Unsupported answer for Docker install: $install_docker_choice" ;;
esac

"$SCRIPT_DIR/check-docker-macos.sh"
...
write_env_value "$ROOT_DIR/.env" "AUTOSHOP_HOME" "$runtime_root"
write_env_value "$ROOT_DIR/.env" "ENV_DIR" "$runtime_root/env/$environment"
write_env_value "$ROOT_DIR/.env" "DATA_ROOT" "$runtime_root/data/$environment"
write_env_value "$ROOT_DIR/.env" "BACKUP_ROOT" "$runtime_root/backups"
write_env_value "$ROOT_DIR/.env" "DOCKER_DEFAULT_PLATFORM" "$DOCKER_DEFAULT_PLATFORM"
...
write_env_value "$ROOT_DIR/.env" "NGINX_HTTP_PORT" "${NGINX_HTTP_PORT:-8088}"
...
case "$environment" in
  staging) run_logged "$ROOT_DIR/scripts/deploy/deploy-staging.sh" ;;
  prod) run_logged "$ROOT_DIR/scripts/deploy/deploy-prod.sh" ;;
esac
